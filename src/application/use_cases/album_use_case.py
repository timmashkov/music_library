...
class AlbumUseCase(Singleton):
    ...
    async def get_item(self, uuid: str | UUID) -> Album | None:
    ...
    async def update_item(
        self, uuid: str | UUID, data: ArtistIncomingData
    ) -> Album | None:
        intel = data.model_dump()
        intel["uuid"] = uuid
        return await self.write_repository.update_item(**intel)

    async def delete_item(self, uuid: str | UUID) -> Album | None:
