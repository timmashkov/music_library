...
class AlbumService(Singleton):
    def __init__(
        self,
        read_repository: AbstractReadRepository = Depends(
            Container.album_read_repository
        ),
        write_repository: AbstractWriteRepository = Depends(
            Container.album_write_repository
        ),
    ) -> None:
        self.read_repository = read_repository
        self.write_repository = write_repository

    async def get_item(self, uuid: str | uuid.UUID) -> Album | None:
        return await self.read_repository.get_item(uuid=uuid)

    async def get_items(self, filters: typing.Any = None) -> typing.List[Album]:
        return await self.read_repository.find(filters=filters)

    async def create_item(self, data: ArtistIncomingData) -> Album | None:
    ...
    async def delete_item(self, uuid: str | uuid.UUID) -> Album | None:
